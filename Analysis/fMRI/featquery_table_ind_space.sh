		...
		for cope in 1 2 3 4 5 6 8; do
			...
			for mask in CA1 CA3DG SUB ERC PRC PHC; do
				for hemi in left right; do
					echo ${mask}
					MEAN_VAL=`cat ${SECOND_LEVEL_PATH}/${subject}/${task}_ENC.gfeat/cope${cope}.feat/${hemi}_${mask}/report.txt | grep stats/pe1 | awk '{print $1 "; " $6}'` # "# featdir" value (column 1) and the mean (column 6). median is column 7
					echo ${MEAN_VAL}
					echo "${subject};${task};${cope};${hemi}_${mask};${hemi};${mask};${MEAN_VAL}" >> featquery_table.csv
				done
			done
		done
	done
