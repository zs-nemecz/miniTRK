...
for subject in `cat $varname`; do
			echo ${subject}
        		...
        		currentjob="transform_MTL_mask_to_meanFunc_${subject}.job"
        		echo ${currentjob}
        		# copy the full template script into the participant specific script
        		cp transform_masks.job ${currentjob}
        		# edit the job file
        		sed -i "s|%PARTICIPANT%|${subject}|g" $currentjob
			
			# some jobs failed preiosly because of out of memory error
			sbatch -p quick --mem 1GB --job-name transform_${subject} ${currentjob}
			#rm $currentjob
done
