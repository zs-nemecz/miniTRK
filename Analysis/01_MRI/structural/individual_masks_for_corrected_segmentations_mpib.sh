...
IOPATH="../../../Results/01_MRI/ASHS/MPIB_Atlas/MPIB_wholebrain_T1/corrected"
...
do
    ...
    ## treshold image according to defined values and binarize
    	...
    	if test -f "$LFILE"; then
			...
			-thr ${thr[$mask]} -uthr ${uthr[$mask]} -bin ${IOPATH}/masks/${subj}_left_${mask}_mask
		fi
    	...
    	if test -f "$RFILE"; then
			...
			-thr ${thr[$mask]} -uthr ${uthr[$mask]} -bin ${IOPATH}/masks/${subj}_right_${mask}_mask
		fi
